...
export function useCreatePatient() {
  ...
  return useMutation({
    mutationFn: (data: Parameters<typeof createPatientFetch>[0]) =>
      createPatientFetch(data),
    onSuccess: () => {
...
export function useAddCaregiver(patientId: string) {
  ...
  return useMutation({
    mutationFn: (caregiverId: number) =>
      addCaregiverFetch(patientId, caregiverId),
    onSuccess: () => {
...
export function useRemoveCaregiver(patientId: string) {
  ...
  return useMutation({
    mutationFn: (assignmentId: string) =>
      removeCaregiverFetch(patientId, assignmentId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: patientKeys.detail(patientId) })
    },
  })
...
export function useRemoveEmergencyContact(patientId: string) {
  ...
  return useMutation({
    mutationFn: (contactId: string) =>
      removeEmergencyContactFetch(patientId, contactId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: patientKeys.detail(patientId) })
    },
  })
