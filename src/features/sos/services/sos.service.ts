...
export async function resolveSosAlertApi(
  id: string,
...
export async function getSosSummaryApi(
  ...
  const acknowledged = allData.results.filter(
    (a) => a.status === "acknowledged"
  ).length
  const resolvedToday = allData.results.filter(
    (a) =>
      a.status === "resolved" && a.resolved_at && a.resolved_at >= todayStart
  ).length
