...
const SHIFT_STATUS_VARIANTS: Record<
  string,
  "default" | "secondary" | "destructive" | "outline"
> = {
  scheduled: "outline",
...
const SOS_STATUS_VARIANTS: Record<
  string,
  "default" | "secondary" | "destructive" | "outline"
> = {
  active: "destructive",
...
export function ClinicDetailView({ id }: ClinicDetailViewProps) {
  ...
  const activeShifts =
    shiftsData?.shifts?.filter((s) => s.status === "in_progress").length ?? 0
  const activeSos =
    sosData?.alerts?.filter((a) => a.status === "active").length ?? 0
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  router.push(`/users?role=caregiver&clinicId=${id}`)
                }
              >
                Ver todos
              </Button>
                    ...
                    {shiftsData.shifts.map((s) => (
                      <TableRow key={s.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              SHIFT_STATUS_VARIANTS[s.status] ?? "outline"
                            }
                          >
                    ...
                    {sosData.alerts.map((alert) => (
                      <TableRow key={alert.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              SOS_STATUS_VARIANTS[alert.status] ?? "outline"
                            }
                          >
