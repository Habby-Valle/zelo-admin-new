...
export async function fetchShifts(params?: ShiftFilters): Promise<{ shifts: ShiftDetail[]; total: number }> {
  ...
  const data = await apiFetchClient<PaginatedResponse<ApiShift>>(`/shifts/${query ? `?${query}` : ""}`)
...
export async function updateShiftFetch(id: number, data: Partial<{
  caregiver_id: number
  clinic_id: number | null
  start: string
  end: string
...
export async function addShiftPatientFetch(shiftId: number, patientId: number): Promise<ShiftPatient> {
...
export async function removeShiftPatientFetch(shiftId: number, assignmentId: string): Promise<void> {
