...
function mapPlan(api: ApiPlan): Plan {
  return {
    ...
    yearly_price: api.yearly_price != null ? parseFloat(api.yearly_price) : null,
...
export async function fetchPlans(params?: {
  ...
  const data = await apiFetchClient<PaginatedResponse<ApiPlan>>(
    `/plans/${query ? `?${query}` : ""}`
  );
...
export async function fetchBenefits(): Promise<PlanBenefit[]> {
  const data =
    await apiFetchClient<{ id: number; key: string; label: string; description: string }[]>(
      "/benefits/"
    );
