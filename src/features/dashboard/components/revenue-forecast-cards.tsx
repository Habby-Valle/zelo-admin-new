  ...
  return (
    <div className="grid gap-4 md:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{formatCurrency(metrics.expected30Days)}</div>
          <p className="text-xs text-muted-foreground">{metrics.trialSubscriptions} em trial</p>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Forecast 6 meses</CardTitle>
          <BarChart3 className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
            {formatCurrency(metrics.forecast.reduce((sum, f) => sum + f.revenue, 0))}
          </div>
