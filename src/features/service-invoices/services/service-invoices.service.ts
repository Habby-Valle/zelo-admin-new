...
import type { ServiceInvoice, ServiceInvoicePage, ServiceInvoiceStats, ServiceInvoiceStatus } from "../types";

function mapInvoice(r: Record<string, unknown>): ServiceInvoice {
  return {
    id: String(r.id ?? ""),
    ...
    status: (r.status as ServiceInvoiceStatus) ?? "pending",
...
function mapLineItem(r: Record<string, unknown>): ServiceInvoice["items"][number] {
  return {
    id: String(r.id ?? ""),
    date: String(r.date ?? ""),
...
export async function fetchServiceInvoiceById(id: string): Promise<ServiceInvoice | null> {
  try {
...
export async function updateServiceInvoiceStatus(id: string, status: "paid" | "cancelled"): Promise<void> {
...
export async function fetchServiceInvoiceStats(): Promise<ServiceInvoiceStats> {
